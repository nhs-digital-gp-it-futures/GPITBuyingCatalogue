...
namespace NHSD.GPIT.BuyingCatalogue.WebApp.UnitTests.Areas.Order.Controllers
{
    public static class DeleteAdditionalServiceControllerTests
    {
        ...
        {
            var expectedViewData = new DeleteAdditionalServiceModel(internalOrgId, order.CallOffId, catalogueItemId, catalogueItemName, order.Description);

            orderServiceMock.Setup(s => s.GetOrderThin(order.CallOffId, internalOrgId)).ReturnsAsync(order);

            var actualResult = await controller.DeleteAdditionalService(internalOrgId, order.CallOffId, catalogueItemId, catalogueItemName);
