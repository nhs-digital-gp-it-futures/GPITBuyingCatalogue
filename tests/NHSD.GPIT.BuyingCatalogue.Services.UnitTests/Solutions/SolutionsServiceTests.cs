...
namespace NHSD.GPIT.BuyingCatalogue.Services.UnitTests.Solutions
{
    public static class SolutionsServiceTests
    {
        ...
        [Theory]
        [CommonAutoData]
        public static async Task SaveApplicationType_InvalidModel_ThrowsException(SolutionsService service)
        {
            var actual = await Assert.ThrowsAsync<ArgumentNullException>(
                () => service.SaveApplicationType(new CatalogueItemId(100000, "001"), null));

            actual.ParamName.Should().Be("applicationTypes");
        }

        [Theory]
        [InMemoryDbAutoData]
        public static async Task SaveApplicationType_UpdatesDatabase(
            [Frozen] BuyingCatalogueDbContext context,
            Solution solution,
            ...
            SolutionsService service)
        {
            ...
            await service.SaveApplicationType(solution.CatalogueItemId, applicationType);

            var actual = await context.Solutions.AsQueryable()
                .FirstAsync(s => s.CatalogueItemId == solution.CatalogueItemId);

            actual.ApplicationType.Should().Be(JsonSerializer.Serialize(applicationType));
        }
        ...
        public static async Task DeleteApplicationType_UpdatesDatabase(
            ...
            ApplicationTypes appliationType,
            [Frozen] BuyingCatalogueDbContext context,
            SolutionsService service)
        {
            appliationType.ClientApplicationTypes =
                new HashSet<string> { "browser-based", "native-mobile", "native-desktop" };
            catalogueSolution.ApplicationType = JsonSerializer.Serialize(appliationType);
            ...
            var actualApplicationType = JsonDeserializer.Deserialize<ApplicationTypes>(actual.ApplicationType);

            actualApplicationType.ClientApplicationTypes.Any(c => c.Equals("browser-based")).Should().BeFalse();
            actualApplicationType.ClientApplicationTypes.Any(c => c.Equals("native-mobile")).Should().BeTrue();
            actualApplicationType.ClientApplicationTypes.Any(c => c.Equals("native-desktop")).Should().BeTrue();
        }
        ...
        {
            applicationTypes.ClientApplicationTypes = new HashSet<string>
            {
            ...
            updatedApplicationType.NativeDesktopAdditionalInformation.Should()
                .Be(applicationTypes.NativeDesktopAdditionalInformation);
            updatedApplicationType.NativeDesktopHardwareRequirements.Should()
                .Be(applicationTypes.NativeDesktopHardwareRequirements);
            updatedApplicationType.NativeDesktopMemoryAndStorage.Should()
                .BeEquivalentTo(applicationTypes.NativeDesktopMemoryAndStorage);
            updatedApplicationType.NativeDesktopMinimumConnectionSpeed.Should()
                .Be(applicationTypes.NativeDesktopMinimumConnectionSpeed);
            updatedApplicationType.NativeDesktopOperatingSystemsDescription.Should()
                .Be(applicationTypes.NativeDesktopOperatingSystemsDescription);
            updatedApplicationType.NativeDesktopThirdParty.Should()
                .BeEquivalentTo(applicationTypes.NativeDesktopThirdParty);

            // Mobile or Tablet
            updatedApplicationType.MobileConnectionDetails.Should()
                .BeEquivalentTo(applicationTypes.MobileConnectionDetails);
            updatedApplicationType.MobileMemoryAndStorage.Should()
                .BeEquivalentTo(applicationTypes.MobileMemoryAndStorage);
            updatedApplicationType.MobileOperatingSystems.Should()
                .BeEquivalentTo(applicationTypes.MobileOperatingSystems);
            updatedApplicationType.MobileThirdParty.Should().BeEquivalentTo(applicationTypes.MobileThirdParty);
            updatedApplicationType.NativeMobileAdditionalInformation.Should()
                .Be(applicationTypes.NativeMobileAdditionalInformation);
            updatedApplicationType.NativeMobileFirstDesign.Should().Be(applicationTypes.NativeMobileFirstDesign);
            updatedApplicationType.NativeMobileHardwareRequirements.Should()
                .Be(applicationTypes.NativeMobileHardwareRequirements);
        }

        [Theory]
        [CommonAutoData]
        public static void Remove_Desktop_ApplicationType_RemovesDesktopEntries(ApplicationTypes applicationType)
        {
            applicationType.ClientApplicationTypes = new HashSet<string>
            {
                ApplicationType.BrowserBased.AsString(EnumFormat.EnumMemberValue),
                ApplicationType.Desktop.AsString(EnumFormat.EnumMemberValue),
                ApplicationType.MobileTablet.AsString(EnumFormat.EnumMemberValue),
            ...
            // Browser Based
            updatedApplicationType.AdditionalInformation.Should().Be(applicationType.AdditionalInformation);
            updatedApplicationType.BrowsersSupported.Should().BeEquivalentTo(applicationType.BrowsersSupported);
            updatedApplicationType.HardwareRequirements.Should().Be(applicationType.HardwareRequirements);
            updatedApplicationType.MinimumConnectionSpeed.Should().Be(applicationType.MinimumConnectionSpeed);
            updatedApplicationType.MinimumDesktopResolution.Should().Be(applicationType.MinimumDesktopResolution);
            updatedApplicationType.MobileFirstDesign.Should().Be(applicationType.MobileFirstDesign);
            updatedApplicationType.MobileResponsive.Should().Be(applicationType.MobileResponsive);
            updatedApplicationType.Plugins.Should().BeEquivalentTo(applicationType.Plugins);
            ...
            // Mobile or Tablet
            updatedApplicationType.MobileConnectionDetails.Should()
                .BeEquivalentTo(applicationType.MobileConnectionDetails);
            updatedApplicationType.MobileMemoryAndStorage.Should()
                .BeEquivalentTo(applicationType.MobileMemoryAndStorage);
            updatedApplicationType.MobileOperatingSystems.Should()
                .BeEquivalentTo(applicationType.MobileOperatingSystems);
            updatedApplicationType.MobileThirdParty.Should().BeEquivalentTo(applicationType.MobileThirdParty);
            updatedApplicationType.NativeMobileAdditionalInformation.Should()
                .Be(applicationType.NativeMobileAdditionalInformation);
            updatedApplicationType.NativeMobileFirstDesign.Should().Be(applicationType.NativeMobileFirstDesign);
            updatedApplicationType.NativeMobileHardwareRequirements.Should()
                .Be(applicationType.NativeMobileHardwareRequirements);
        }

        [Theory]
        [CommonAutoData]
        public static void Remove_Mobile_ApplicationType_RemovesMobileEntries(ApplicationTypes applicationType)
        {
            applicationType.ClientApplicationTypes = new HashSet<string>
            {
                ApplicationType.BrowserBased.AsString(EnumFormat.EnumMemberValue),
            ...
            // Desktop
            updatedApplicationType.NativeDesktopAdditionalInformation.Should()
                .Be(applicationType.NativeDesktopAdditionalInformation);
            updatedApplicationType.NativeDesktopHardwareRequirements.Should()
                .Be(applicationType.NativeDesktopHardwareRequirements);
            updatedApplicationType.NativeDesktopMemoryAndStorage.Should()
                .BeEquivalentTo(applicationType.NativeDesktopMemoryAndStorage);
            updatedApplicationType.NativeDesktopMinimumConnectionSpeed.Should()
                .Be(applicationType.NativeDesktopMinimumConnectionSpeed);
            updatedApplicationType.NativeDesktopOperatingSystemsDescription.Should()
                .Be(applicationType.NativeDesktopOperatingSystemsDescription);
            updatedApplicationType.NativeDesktopThirdParty.Should()
                .BeEquivalentTo(applicationType.NativeDesktopThirdParty);
