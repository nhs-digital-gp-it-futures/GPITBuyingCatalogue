...
public class LoginTests : BaseTest
{
    ...
    public async Task OrderWithCatalogueSolutionAndAssociatedService()
    {
        ...
        await Page.GetByRole(AriaRole.Link, new() { Name = "Add a bespoke milestone" }).ClickAsync();
        await Page.GetByRole(AriaRole.Textbox, new() { Name = "Milestone name" }).FillAsync("milestone 1as");
        await Page.GetByRole(AriaRole.Textbox, new() { Name = "Milestone payment trigger" }).FillAsync("fasfsadga");
        await Page.GetByRole(AriaRole.Button, new() { Name = "Save and continue" }).ClickAsync();
        await Page.GetByRole(AriaRole.Button, new() { Name = "Save and continue" }).ClickAsync();
        ...
        await Page.GetByRole(AriaRole.Link, new() { Name = "Add a milestone" }).ClickAsync();
        await Page.GetByLabel("Associated service name").SelectOptionAsync(new[] { "Engineering" });
        await Page.GetByRole(AriaRole.Textbox, new() { Name = "Milestone name" }).FillAsync("milestone 2a");
        await Page.GetByRole(AriaRole.Textbox, new() { Name = "Milestone payment trigger" }).FillAsync("dfsafas");
        await Page.GetByRole(AriaRole.Button, new() { Name = "Save and continue" }).ClickAsync();
        await Page.GetByRole(AriaRole.Button, new() { Name = "Save and continue" }).ClickAsync();
        ...
        await Page.GetByRole(AriaRole.Link, new() { Name = "Add a requirement" }).ClickAsync();
        await Page.GetByLabel("Associated service name").SelectOptionAsync(new[] { "Engineering" });
        await Page.GetByRole(AriaRole.Textbox, new() { Name = "What specific requirements do" }).FillAsync("fasdafad");
