...
from bcwagtail.frameworks.choices import DAY_CHOICES


class OperatingHours(models.Model):
    """
    A Django model to capture operating hours for a Framework
    """
    ...
    closed = models.BooleanField(
        "Closed?", blank=True, help_text="Tick if framework is closed on this day"
    )
...
class FrameworkOperatingHours(Orderable, OperatingHours):
    """
    A model creating a relationship between the OperatingHours and Framework
    Note that unlike releasenotesPersonRelationship we don't include a ForeignKey to
    OperatingHours as we don't need that relationship (e.g. any Framework open
    ...
    name to access it from the FrameworkPage admin
    """

    framework = ParentalKey(
        "FrameworkPage", related_name="hours_of_operation", on_delete=models.CASCADE
    )


class FrameworksIndexPage(Page):
    """
    ...
    # Only FrameworkPage objects can be added underneath this index page
    subpage_types = ["FrameworkPage"]
    ...
    def get_context(self, request):
        context = super(FrameworksIndexPage, self).get_context(request)
        context["frameworks"] = (
            FrameworkPage.objects.descendant_of(self).live().order_by("title")
        )
        return context
...
class FrameworkPage(Page):
    """
    Detail for a specific bakery framework.
    """
    ...
    # Determines if the framework is currently open. It is timezone naive
    def is_open(self):
        now = datetime.now()
        ...
        try:
            ...
            return True
        except FrameworkOperatingHours.DoesNotExist:
            return False
    ...
    def get_context(self, request):
        context = super(FrameworkPage, self).get_context(request)
        context["lat"] = self.lat_long.split(",")[0]
        context["long"] = self.lat_long.split(",")[1]
        context["google_map_api_key"] = settings.GOOGLE_MAP_API_KEY
        return context

    # Can only be placed under a FrameworksIndexPage object
    parent_page_types = ["FrameworksIndexPage"]
