...
from bcwagtail.base.models import FooterText, HomePage, Person, StandardPage
from bcwagtail.releasenotes.models import releasenotesIndexPage, releasenotesPage
from bcwagtail.newfeatures.models import (
    NewFeatureIngredient,
    NewFeaturePage,
    NewFeaturesIndexPage,
    NewFeatureType,
    Country,
)
from bcwagtail.frameworks.models import FrameworkPage, FrameworksIndexPage
...
class Command(BaseCommand):
    ...
    def create_pages(self, page_count):
        self.stdout.write("Creating newfeature pages...")
        newfeatures_index = NewFeaturesIndexPage.objects.live().first()
        for _ in range(page_count):
            title = self.make_title()
            newfeatures_index.add_child(
                instance=NewFeaturePage(
                    title=title,
                    slug=slugify(title),
                    introduction=lorem_ipsum.paragraph(),
                    newfeature_type=self.get_random_model(NewFeatureType),
        ...
        self.stdout.write("Creating framework pages...")
        framework_index = FrameworksIndexPage.objects.live().first()
        for _ in range(page_count):
            title = self.make_title()
            framework_index.add_child(
                instance=FrameworkPage(
                    title=title,
                    slug=slugify(title),
                    introduction=lorem_ipsum.paragraph(),
                    image=self.get_random_model(Image),
        ...
        self.stdout.write("Creating releasenotes pages...")
        releasenotes_index = releasenotesIndexPage.objects.live().first()
        for _ in range(page_count):
            title = self.make_title()
            releasenotes_index.add_child(
                instance=releasenotesPage(
                    title=title,
                    slug=slugify(title),
                    introduction=lorem_ipsum.paragraph(),
                    body=self.fake_stream_field(),
    ...
    def create_snippets(self, snippet_count):
        ...
        self.stdout.write("Creating newfeature ingredients...")
        for _ in range(snippet_count):
            NewFeatureIngredient.objects.create(name=self.make_title())

        self.stdout.write("Creating newfeature types...")
        for _ in range(snippet_count):
            NewFeatureType.objects.create(title=self.make_title())
