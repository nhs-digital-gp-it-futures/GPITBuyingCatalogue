...
public class ImportController : Controller
{
    ...
    public async Task<IActionResult> ImportGpPracticeList(ImportGpPracticeListModel model)
    {
        ...
        return RedirectToAction(
            nameof(ImportGpPracticeListConfirmation),
            new { model.CsvUrl });
    }

    [HttpGet("gp-practice-list/confirmation")]
    public async Task<IActionResult> ImportGpPracticeListConfirmation(string csvUrl)
    {
        var user = await usersService.GetUser(User.UserId());

        return View(new ImportGpPracticeListConfirmationModel(csvUrl, user.Email));
    }
}
