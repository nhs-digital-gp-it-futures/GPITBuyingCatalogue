...
public class FrameworksController : Controller
{
    ...
    public async Task<IActionResult> Add(AddEditFrameworkModel model)
    {
        ...
        await frameworkService.AddFramework(model.Name, model.FundingTypes.Where(x => x.Selected).Select(x => x.Value), model.SupportsFoundationSolution);
    ...
    public async Task<IActionResult> Edit(string frameworkId, AddEditFrameworkModel model)
    {
        ...
        await frameworkService.UpdateFramework(frameworkId, model.Name, model.FundingTypes.Where(x => x.Selected).Select(x => x.Value), model.SupportsFoundationSolution);
