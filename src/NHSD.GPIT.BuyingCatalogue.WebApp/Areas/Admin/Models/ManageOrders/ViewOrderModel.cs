    ...
    public class ViewOrderModel : NavBaseModel
    {
        public ViewOrderModel(EntityFramework.Ordering.Models.Order order)
        {
            ...
            OrderItems = order.OrderItems?.Select(oi => new AdminViewOrderItem
            {
                ...
                Framework = oi.CatalogueItem.CatalogueItemType == CatalogueItemType.Solution
                    ? oi.CatalogueItem.Solution.FrameworkSolutions.FirstOrDefault(fs => fs.FrameworkId != "COVID").Framework.ShortName
                    : oi.CatalogueItem.CatalogueItemType == CatalogueItemType.AdditionalService
                        ? oi.CatalogueItem.AdditionalService.Solution.FrameworkSolutions.FirstOrDefault(fs => fs.FrameworkId != "COVID")?.Framework.ShortName
                        : oi.CatalogueItem.AssociatedService.CatalogueItem.Solution.FrameworkSolutions.FirstOrDefault(fs => fs.FrameworkId != "COVID").Framework.ShortName,
            });
        }
