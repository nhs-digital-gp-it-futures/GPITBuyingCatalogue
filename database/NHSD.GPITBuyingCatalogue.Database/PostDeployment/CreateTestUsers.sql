...
BEGIN
    ...
    DECLARE @aliceOrganisationId AS uniqueidentifier = (SELECT TOP (1) OrganisationId FROM organisations.Organisations WHERE PrimaryRoleId = @ccgRoleId ORDER BY OdsCode);
    ...
    DECLARE @bobOrganisationId AS uniqueidentifier = (SELECT OrganisationId FROM organisations.Organisations WHERE PrimaryRoleId = @executiveAgencyRoleId);
    ...
    DECLARE @sueOrganisationId AS uniqueidentifier = (SELECT TOP (1) OrganisationId FROM organisations.Organisations WHERE PrimaryRoleId = @ccgRoleId AND OdsCode = @hullCCGOdsCode);
