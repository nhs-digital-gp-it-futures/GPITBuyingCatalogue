...
INSERT INTO @odsOrganisationRelationships
	...
	VALUES (999999000, 'RE4',                'K84049',               '10Q',                 1         ),
		   ...
		   (999999282, 'RE5',                '0CX',                  '0CX',                 1         )

INSERT INTO [ods_organisations].[OrganisationRelationships]
    ([Id], [RelationshipTypeId], [TargetOrganisationId], [OwnerOrganisationId], [IsActive])
SELECT
    [oor].[Id], [oor].[RelationshipTypeId], [oor].[TargetOrganisationId], [oor].[OwnerOrganisationId], [oor].[IsActive]
FROM
    @odsOrganisationRelationships [oor]
WHERE
    NOT EXISTS (
        SELECT 1
        FROM [ods_organisations].[OrganisationRelationships] [or]
        WHERE [or].[RelationshipTypeId] = [oor].[RelationshipTypeId] 
		  AND [or].[TargetOrganisationId] = [oor].[TargetOrganisationId] 
		  AND [or].[OwnerOrganisationId] = [oor].[OwnerOrganisationId] 
		  AND [or].[IsActive] = 1
    )
    AND EXISTS (
        SELECT 1
        FROM [ods_organisations].[OdsOrganisations] [oo1]
        JOIN [ods_organisations].[OdsOrganisations] [oo2]
            ON [oo1].[Id] = [oor].[TargetOrganisationId]
           AND [oo2].[Id] = [oor].[OwnerOrganisationId]
    );
